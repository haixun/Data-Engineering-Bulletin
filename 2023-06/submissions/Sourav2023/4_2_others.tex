...
\noindent
\textbf{(3) Counterfactual}: \textbf{Global counterfactual explainer} \cite{Global-counter} finds a candidate set of counterfactuals using vertex re-inforced random walk. It then uses a greedy strategy to select the top $k$ counterfactuals from the candidate set as global explanations. We explain it in more detail in Sec. \ref{sec::Counterfactual-search}. %To the best of our knowledge, there is only one Global counterfactual explainer \cite{Global-counter} and we explain it in more detail in section \ref{sec::Counterfactual}.
